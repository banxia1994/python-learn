...
class BaiduImgDownloader(object):
    ...
    def __init__(self, word, dirpath=None, processNum=30,imgsNUm = 500):
        if " " in word:
        ...
        if not dirpath:
            dirpath = os.path.join(sys.path[0], 'pic3')
            dirpath = os.path.join(dirpath,dir_)
        ...
        self.imgsNum = 0 #图片的数量
        self.maxImgs = imgsNUm #每类图片最大数量
    ...
    def start(self):
        ...
        while self.queue.qsize():
            imgs = self.queue.get()
            imnum = self.__getImgsNum(len(imgs))


            self.pool.map_async(self.__downImg, imgs)
            if imnum > self.maxImgs:
                break
    ...
    def __buildUrls(self):
        ...
        # try:
        #     html = requests.get(url.format(word=word, pn=0), timeout = 15).content.decode('utf-8')
        ...
        urls = [url.format(word=word, pn=x)
                for x in range(0, 10000, 60)]   #截止到哪里,这里会控制图片的多少
